...
export default registerAs('database', (): DatabaseConfig => {
  ...
  let ssl: boolean | { rejectUnauthorized: boolean; ca?: string; cert?: string; key?: string } = false;
  ...
  let logging: boolean | ('query' | 'error' | 'schema' | 'warn' | 'info' | 'log')[] = false;
  ...
  if (loggingEnv === 'true') {
    ...
    logging = loggingEnv.split(',') as ('query' | 'error' | 'schema' | 'warn' | 'info' | 'log')[];
  }
